<x-admin-layout>
    <div class="mb-6">
        <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Kategori</h1>
        <p class="text-xs text-slate-400 mt-1">Ubah nama kategori yang dipilih untuk memperbarui klasifikasi pengaduan.</p>
    </div>
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $category->name) }}" 
                    class="block w-full rounded-xl border-slate-200 px-4 py-3 text-sm text-slate-800 shadow-sm transition-all duration-150 focus:border-slate-900 focus:ring focus:ring-slate-900/5" required>
