<nav x-data="{ open: false }" class="bg-white/70 backdrop-blur-md border-b border-slate-200/60 relative z-30">
        ...
        <div class="flex justify-between h-20">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link :href="route('dashboard')" :active="Request::is('dashboard') || Request::is('user/dashboard')" 
                        class="text-sm font-bold tracking-tight transition duration-150 {{ (Request::is('dashboard') || Request::is('user/dashboard')) ? 'text-indigo-600 border-indigo-600' : 'text-slate-500 hover:text-indigo-600 hover:border-slate-300' }}">
                        {{ __('Dashboard') }}
                    </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link href="#" class="font-semibold text-slate-400 cursor-not-allowed">
                            {{ __('Profil (Disabled)') }}
                        </x-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-b border-slate-200">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('dashboard')" :active="Request::is('dashboard') || Request::is('user/dashboard')"
                class="font-bold {{ (Request::is('dashboard') || Request::is('user/dashboard')) ? 'text-indigo-600 bg-indigo-50/50 border-indigo-600' : 'text-slate-600' }}">
                {{ __('Dashboard') }}
        ...
        <div class="pt-4 pb-1 border-t border-slate-200 bg-slate-50/50">
            ...
            <div class="mt-3 space-y-1">
                <x-responsive-nav-link href="#" class="font-semibold text-slate-400">
                    {{ __('Profil (Disabled)') }}
                </x-responsive-nav-link>
